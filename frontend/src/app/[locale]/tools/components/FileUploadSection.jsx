  ...
  return (
    <LabelInputContainer>
      <Label className="flex items-center text-sm font-medium mb-1.5">
        ...
        {t("fields.referenceMaterials.label")} 
      </Label>
      ...
      {uploadedFiles.length > 0 ? (
          ...
          {uploadedFiles.map(file => (
            <div key={file.id} className="flex items-center justify-between mb-2 last:mb-0">
              <div className="flex items-center overflow-hidden">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2 flex-shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span className={`text-sm truncate ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                  {file.name} {file.isFromClassroom && `(${t("fields.referenceMaterials.fromClassroom")})`} 
                </span>
              </div>
              <button
                type="button"
        ...
        <div className="flex flex-col space-y-3">
          <div className={`relative border-2 border-dashed rounded-lg p-4 text-center ${theme === 'dark' ? 'border-zinc-700 hover:border-zinc-500' : 'border-gray-300 hover:border-gray-400'}`}>
            ...
            <div className="space-y-1 text-center">
              ...
              <div className="text-sm text-gray-500">
                <span className="font-medium">{t("fields.referenceMaterials.upload")}</span> 
                {t("fields.referenceMaterials.dragAndDrop")} 
              </div>
              <p className="text-xs text-gray-500">{t("fields.referenceMaterials.fileRequirements")}</p> 
            </div>
          </div>
          
          <div className="text-center">
            <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{t("fields.referenceMaterials.or")}</span> 
          </div>
          ...
          >
            {t("fields.referenceMaterials.selectFromClassroom")} 
          </button>
        </div>
      )}
      ...
      {isProcessingFile && (
        <div className="mt-2 flex items-center justify-center">
          ...
          <span className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{t("fields.referenceMaterials.processing")}</span> 
        </div>
      )}
    </LabelInputContainer>
  );
