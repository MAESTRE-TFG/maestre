...
export default function ProfileEdit() {
  ...
  const handleUpdate = useCallback(async () => {
    try {
      const response = await axios.put(`http://localhost:8000/api/users/${user.id}/update/`, formData, {
        headers: {
  ...
  const handleDelete = useCallback(async () => {
    const confirmationMessage = `¿Estás seguro de que deseas eliminar tu cuenta de MAESTRE? Esto eliminará tambien todos tus cursos y material subido a la plataforma. Esta acción NO se puede desahacer.\n\nPara borrar tu cuenta, escribe antes tu nombre de usuario (${user.username}):`;
        ...
        <div className="min-h-screen flex flex-col justify-center items-center py-12 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                <h1 className={cn(
                    ...
                    Hello <span style={{ fontFamily: "'Alfa Slab One', sans-serif" }}>
                        {user ? user.name : ""}
                    </span> !
                </h1>
                <h4 className={cn(
                    ...
                    <div className={cn(
                        ...
                        <div className="my-8">
                            <LabelInputContainer className="mb-4">
                                ...
                                <p className={cn(theme === "dark" ? "text-white" : "text-black")} style={{ fontFamily: "inherit" }}>{user?.username}</p>
                            </LabelInputContainer><br></br>
                            <LabelInputContainer className="mb-4">
                                <Label>Email Address</Label>
                                <p className={cn(theme === "dark" ? "text-white" : "text-black")} style={{ fontFamily: "inherit" }}>{user?.email}</p>
                            </LabelInputContainer><br></br>
                            <LabelInputContainer className="mb-4">
                                <Label>Name</Label>
                                <p className={cn(theme === "dark" ? "text-white" : "text-black")} style={{ fontFamily: "inherit" }}>{user?.name}</p>
                            </LabelInputContainer><br></br>
                            <LabelInputContainer className="mb-4">
                                <Label>Surname</Label>
                                <p className={cn(theme === "dark" ? "text-white" : "text-black")} style={{ fontFamily: "inherit" }}>{user?.surname}</p>
                            </LabelInputContainer><br></br>
                                ...
                                <button onClick={handleDelete}
                                    className={cn("relative group/btn block w-full mx-auto rounded-md h-10 font-medium border border-transparent mt-4", 
                                     theme === "dark" ? "text-white bg-gradient-to-br from-zinc-900 to-zinc-900 shadow-[0px_1px_0px_0px_var(--zinc-800)_inset,0px_-1px_0px_0px_var(--zinc-800)_inset]" 
                                     : "text-black bg-gradient-to-br from-white to-neutral-100 shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] border border-blue-300"
                                    )}
                                    type="submit">
